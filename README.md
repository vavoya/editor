...
### 주의!!
**Safari** 환경을 기준으로 개발 되었으며, **Chrome** 으로 실행할 시 정상적인 동작이 발생하지 않을 수 있습니다. (확인 결과: Chrome에서는 마우스로 Caret 설정이 안될 뿐, 나머지는 정상 동작 합니다. 브라우저 API가 서로 다르기에 발생하는 것으로 추측됨. 로직상 그럼)
